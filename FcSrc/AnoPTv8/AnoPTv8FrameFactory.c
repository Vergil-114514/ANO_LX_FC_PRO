...
{
    uint8_t _cnt = 0;
    int bufindex = AnoPTv8GetFreeTxBufIndex(ANOPTV8TXPRI_CMD);
...
{
    uint8_t _cnt = 0;
    int bufindex = AnoPTv8GetFreeTxBufIndex(ANOPTV8TXPRI_INFO);
...
void AnoPTv8SendCmdInfo(const uint16_t linktype, const uint8_t daddr, const uint16_t cmd)
{
    ...
    if (bufindex >= 0)
    {
        ...
        for(int i=0; i<3; i++)
            AnoPTv8TxBuf[bufindex].dataBuf.frame.data[_cnt++] = _p.cmd.cid[i];
        for(int i=0; i<8; i++)
            AnoPTv8TxBuf[bufindex].dataBuf.frame.data[_cnt++] = _p.cmd.valType[i];
...
void AnoPTv8SendFrame0x0D(const uint16_t linktype, const uint8_t daddr, const float vbat, const float ibat, const float vfc, const float ifc)
{
	...
	_sbuf[_cnt++] = BYTE0(_tmp);
	_sbuf[_cnt++] = BYTE1(_tmp);
	AnoPTv8SendBuf(linktype, daddr, 0x0D, ANOPTV8TXPRI_DATA, _sbuf, _cnt);
}
