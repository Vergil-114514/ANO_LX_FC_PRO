...
static void AnoPTv8MirrorLxFrame(const _un_frame_v8 *p)
{
    ...
    memcpy(mirrorFrame.rawBytes, p->rawBytes, frameLen);
    mirrorFrame.frame.ddevid = ANOPTV8DEVID_SWJ;
    AnoPTv8CalFrameCheck(&mirrorFrame);
    AnoPTv8HwSendBytes(LT_D_SWJ, mirrorFrame.rawBytes, frameLen);
}
