...
void DrvPwmOutInit(void)
{
    ...
    uint32_t hz_set = ACCURACY *PWM_FRE_HZ;
...
void DrvMotorPWMSet(int16_t pwm[8])
{
    ...
    if (EmergencyStopESC)
    {
		...
		TIM5->CCR3 = PWM_RADIO *(0) + INIT_DUTY;  //5
        TIM5->CCR4 = PWM_RADIO *(0) + INIT_DUTY;  //6
        TIM8->CCR3 = PWM_RADIO *(0) + INIT_DUTY;  //7
        TIM8->CCR4 = PWM_RADIO *(0) + INIT_DUTY;  //8
    }
    else
    {
//		pwm[0] = _val;
...
//		pwm[3] = _val;
        ...
        TIM5->CCR3 = PWM_RADIO *(pwm[4]) + INIT_DUTY;  //5
        TIM5->CCR4 = PWM_RADIO *(pwm[5]) + INIT_DUTY;  //6
        TIM8->CCR3 = PWM_RADIO *(pwm[6]) + INIT_DUTY;  //7
        TIM8->CCR4 = PWM_RADIO *(pwm[7]) + INIT_DUTY;  //8
    }
}
